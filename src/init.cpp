...
void Shutdown()
{
    ...
    RenameThread("PeepMasternode-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: PeepMasternode.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: peepmasternoded.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizepeepmasternodeamount=<n> " + _("Keep N PeepMasternode anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. PeepMasternode is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. PeepMasternode is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("PeepMasternode version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "PeepMasternode server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of PeepMasternode") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart PeepMasternode to complete") << "\n";
    ...
    nAnonymizePeepMasternodeAmount = GetArg("-anonymizepeepmasternodeamount", 0);
    if(nAnonymizePeepMasternodeAmount > 999999) nAnonymizePeepMasternodeAmount = 999999;
    if(nAnonymizePeepMasternodeAmount < 2) nAnonymizePeepMasternodeAmount = 2;
    ...
    LogPrintf("Anonymize PeepMasternode Amount %d\n", nAnonymizePeepMasternodeAmount);
