...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePeepMasternodeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePeepMasternodeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePeepMasternodeAmount = strAnonymizePeepMasternodeAmount.remove(strAnonymizePeepMasternodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePeepMasternodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePeepMasternodeAmount*COIN) nMaxToAnonymize = nAnonymizePeepMasternodeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePeepMasternodeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePeepMasternodeAmount));
        strAnonymizePeepMasternodeAmount = strAnonymizePeepMasternodeAmount.remove(strAnonymizePeepMasternodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePeepMasternodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizePeepMasternodeAmount == 0){
