...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePeepMasternodeAmount"))
        settings.setValue("nAnonymizePeepMasternodeAmount", 1000);
    nAnonymizePeepMasternodeAmount = settings.value("nAnonymizePeepMasternodeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizePeepMasternodeAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizePeepMasternodeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePeepMasternodeAmount:
            return QVariant(nAnonymizePeepMasternodeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizePeepMasternodeAmount:
            nAnonymizePeepMasternodeAmount = value.toInt();
            settings.setValue("nAnonymizePeepMasternodeAmount", nAnonymizePeepMasternodeAmount);
            emit AnonymizePeepMasternodeAmountChanged(nAnonymizePeepMasternodeAmount);
            break;
        default:
            break;
        }
