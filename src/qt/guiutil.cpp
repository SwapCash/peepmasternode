...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("PeepMasternode"))
        return false;
...
{
    // Convert PeepMasternode:// to PeepMasternode:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("PeepMasternode://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "PeepMasternode:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PeepMasternode.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for PeepMasternode.lnk
...
{
    return GetAutostartDir() / "PeepMasternode.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a PeepMasternode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=PeepMasternode\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the PeepMasternode app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add PeepMasternode app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PeepMasternode-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PeepMasternode-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("PeepMasternode-Qt"));
