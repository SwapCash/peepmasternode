...
int nAnonymizePeepMasternodeAmount = 1000;
...
#else
    const char* pszModule = "PeepMasternode";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PeepMasternode
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PeepMasternode
    // Mac: ~/Library/Application Support/PeepMasternode
    // Unix: ~/.PeepMasternode
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PeepMasternode";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "PeepMasternode.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "peepmasternoded.pid"));
